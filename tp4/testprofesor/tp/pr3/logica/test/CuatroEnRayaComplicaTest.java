...
public class CuatroEnRayaComplicaTest extends CuatroEnRayaConecta4Test {
	...
	private boolean preparaColocacionFichaDesplaza(Partida p, Ficha color, int x, int y) throws MovimientoInvalido {
	...
	private void testCuatroEnRayaDesplaza(int posX[], int posY[], int ultima, Ficha color) throws MovimientoInvalido {
		...
		for (int x = 1; x <= 7; ++x) {
			...
			try {
				p.ejecutaMovimiento(getMovimiento(x, Ficha.NEGRA));
				fail("No se debe poder poner tras terminar la partida.");
			} catch(MovimientoInvalido e) {};
		}
	}
	...
	private void pruebaCuatroEnRayaDesplaza(int posX[], int posY[]) throws MovimientoInvalido {
		for (int i = 0; i < posX.length; ++i) {
			posY[i]--;
	...
	// horizontal
	@Test
	public void testCuatroEnRayaHorizontalDesplaza() throws MovimientoInvalido {
	...
	// diagonal /
	@Test
	public void testCuatroEnRayaDiag1Desplaza() throws MovimientoInvalido {
		Tablero t = r.iniciaTablero();
	...
	// diagonal \
	@Test
	public void testCuatroEnRayaDiag2Desplaza() throws MovimientoInvalido {
		Tablero t = r.iniciaTablero();
